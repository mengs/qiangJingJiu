...
PFont myFont= createFont("Adobe heiti Std", 26);
String myStr = "君不见黄河之水天上来奔流到海不复回君不见黄河之水天上来奔流到海不复回"; // total length is 13
...
{ 
  ...
  for ( int i =0; i<myStr.length(); i++)
  {
    ...
    if (px+(py[i]+3)*cam.width>=cam.pixels.length | pix<0)
    {
      py[i]=0;
      pix=pix/(height+2);
    }

    else if (brightness(cam.pixels[pix]) < 140) {
      py[i]-=4;
    }
    // otherwise fall
    else {
